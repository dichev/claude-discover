...
│   └── scripts/usage.mjs           CLI: per-period token usage/cost table (imports src/, so repo-only)
...
- Deep linking (`claude-discover://session?id=…&date=…`): `DeepLink` (`src/main/services/DeepLink.js`) takes a single-instance lock and funnels every launch into the one window — the URL arrives either in the cold-start `process.argv` or in a second launch's argv, forwarded by `second-instance`. A link from a running app is pushed as `deeplink:open-session`; a cold-start link is parsed before the page can listen, so the renderer pulls it on mount (`takeDeepLink`). Either way `App.jsx` jumps to the session's day and selects it; when the id isn't in that period (or the period is empty) it computes a `missing` notice that `Session.jsx` shows in place of the blank detail pane, held back until the scan reports `scanning: false` so a slow scan can't flash "not found". The selection carries its origin: `selectSession(id, link)` takes the link only from the deep-link handler, so `SessionList` tints that row green (`.deep-linked`) and appends "opened from deep link" to its second line, until the next manual pick clears it. **Windows only**: `#register()` points `HKCU\Software\Classes\claude-discover` at `electron.exe <appDir> "%1"` on every launch (skipped for ephemeral `npx` runs, whose dir is deleted later). macOS would need the app packaged as an `.app` bundle plus an `open-url` handler, Linux a `.desktop` file — routing already works on both, only registration is missing.
