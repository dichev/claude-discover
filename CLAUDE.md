...
- `node bin/capture-requests-proxy.mjs --restart` — start/restart the API request-capture proxy in the foreground (does not touch settings.json).
...
│   ├── capture-requests-proxy.mjs  API request-capture logging proxy
...
- Data source: `<CLAUDE_DIR>/projects/**/*.jsonl` (sessions). `CLAUDE_DIR` is resolved in `src/main/paths.js` — user override in `~/.claude-discover.json` (`claudeDir`, plus the source-switcher's `recents`) wins over `$CLAUDE_CONFIG_DIR`, which wins over `~/.claude`. Read/persist that file only via `getConfig()` / `setConfig()` from `paths.js`.
...
- Request capture: `bin/capture-requests-proxy.mjs` is a logging proxy that forwards Claude Code's API traffic verbatim to `api.anthropic.com` and appends one NDJSON record per request (any endpoint, count_tokens included) to `<CLAUDE_DIR>/.claude-discover/requests/<sessionId>.requests.jsonl` — raw request + headers (credentials redacted) + response with headers (SSE streams reassembled by `assembleSSE`). Capture must never fail or delay a request — errors go to `<CLAUDE_DIR>/.claude-discover/proxy.error.log`. Port `41414` (`--port`/`--upstream` overrides used only by tests); at startup it verifies the upstream is reachable and exits with code 2 if not; `--restart` replaces a running instance via the loopback-only `/claude-discover/exit` control route. `GET /claude-discover/ping` is answered directly, never forwarded — `ProxySwitch` probes it for the StatusBar.
  ...
  - The proxy never deletes logs; orphan cleanup is the app's job. After `SessionsService`'s first complete scan, `RequestFile.sweepOrphans` deletes request logs whose transcript no longer exists under `projects/` — existence, not age, is the criterion (so a log captured with the proxy off can't be lost while its session is still browsable), and logs touched within the last day are never swept.
...
- `getSwitchStatus(name)` / `activateSwitch(name)` / `deactivateSwitch(name)` — one generic surface for the StatusBar switches, backed by `Switchers` (`src/main/services/switchers/`). Each `*Switch` holds pure feature logic — `status()` plus `activate()`/`deactivate()` that do the work or **throw** a friendly message; `Switchers.#attempt` maps every outcome to fresh status plus an optional `error` string alerted by the tooltip button. A new switch is a new `*Switch` class plus a `#switches` map entry. `StatusSwitch.jsx`'s `useSwitch` polls status every 5s so external settings.json edits show up live; a `StatusSwitch` given a custom `button` label renders an action button that always activates.
  ...
  - **claudedir** — `{ dir }`. Action-style ("Change directory", no off state — deactivate throws): opens a folder picker, persists the choice to `~/.claude-discover.json` and relaunches. The File menu's source-switcher reuses `ClaudeDirSwitch`'s browse/switch logic.
