...
- `node test/scripts/package-mac.mjs` — @macOS refresh `dist/claude-discover.app` (the local unsigned bundle owning the `claude-discover://` scheme) and register it; run the app by opening that bundle.
...
- Session scanning (`src/main/sessions/`): `SessionsScanner` watches the projects dir via chokidar and, after one full walk, serves scans from the watcher-fed `StatCache` instead of re-walking disk; `SessionFile`/`SessionParser` parse individual `.jsonl` files; `MetaCache` caches session metas keyed by size+mtime. `scanner.watch()` waits for a scan before starting chokidar (and resolves once it's live) — chokidar's setup walks the whole tree, and on a cold disk those stats dominate startup; `scan()` itself has no watcher side effect, it just marks the disk free. That first walk therefore never marks the `StatCache` complete (it needs a live watcher); the next scan re-walks a warm tree and marks it.
- `SessionsService.scan(period, { watch })` is the single scan entry point: `watch:true` (UI) streams `update` events per dir batch and returns the cache immediately; `watch:false` (CLIs/tests like `test/scripts/usage.mjs`) awaits the full scan and returns the deduped sessions with no emits.
...
- Request capture: `bin/proxy.mjs` is a logging proxy that forwards Claude Code's API traffic verbatim to `api.anthropic.com` and appends one NDJSON record per request (any endpoint, count_tokens included) to `~/.claude-discover/requests/<sessionId>.requests.jsonl` — raw request + headers (credentials redacted) + response with headers (SSE streams reassembled by `assembleSSE`). The requests dir is **global** (`DATA_DIR` in `bin/proxy.config.js`; its `$CLAUDE_DISCOVER_DIR` override is used only by tests): session ids are unique, so one flat dir serves every Claude dir/config on the machine and the proxy needs no per-dir config, restart, or notion of which Claude dir a request belongs to. Capture must never fail or delay a request — errors go to `~/.claude-discover/proxy.error.log`. Port `41414` (`$CLAUDE_DISCOVER_PORT`/`$CLAUDE_DISCOVER_UPSTREAM` overrides used only by tests); at startup it verifies the upstream is reachable and exits with code 2 if not; `--restart` replaces a running instance via the loopback-only `/claude-discover/exit` control route. `GET /claude-discover/ping` is answered directly, never forwarded — `ProxySwitch` probes it for the StatusBar.
  ...
  - Subagents send the **parent's** session id (their requests land in the parent's log) plus an `x-claude-code-agent-id` header; `RequestFile`'s `agentId` option scopes `read`/`readInstructions` to one agent's records (null = the session's own, so parent views exclude agent-tagged records), while refs still resolve over the whole file. `SessionsService.#requestFile` does the routing: an `agent-<id>` session reads its parent's log filtered by `<id>`. The two sides spell that id differently, so `agentKey` normalizes both — an anonymous Task subagent is `a<16hex>` in the transcript id and the header alike, but a named teammate is `a<name>-<16hex>` in the transcript and `<name>@<team>` in the header.
...
- Deep linking (`claude-discover://session?id=…&date=…`): `DeepLink` (`src/main/services/DeepLink.js`) takes a single-instance lock and funnels every launch into the one window — the URL arrives in the cold-start `process.argv`, in a second launch's argv (`second-instance`), or as macOS's `open-url` event. A link from a running app is pushed as `deeplink:open-session`; one that arrives before the page can listen is parked, and the renderer pulls it on mount (`takeDeepLink`). Either way `App.jsx` jumps to the session's day and selects it; when the id isn't in that period (or the period is empty) it computes a `missing` notice that `Session.jsx` shows in place of the blank detail pane, held back until the scan reports `scanning: false` so a slow scan can't flash "not found". The selection carries its origin: `selectSession(id, link)` takes the link only from the deep-link handler, so `SessionList` tints that row green (`.deep-linked`) and appends "opened from deep link" to its second line, until the next manual pick clears it. Only scheme registration is OS-specific. **@windows**: `#register()` points `HKCU\Software\Classes\claude-discover` at `electron.exe <appDir> "%1"` on every launch (skipped for ephemeral `npx` runs, whose dir is deleted later). **@macOS**: schemes belong to `.app` bundles, so the app ships itself as one — `test/scripts/package-mac.mjs` rebuilds `dist/claude-discover.app` whenever it's older than `out/` and `lsregister`s it (`@electron/packager`, stable id `com.claudediscover.app`, scheme declared via `protocols`), leaving nothing to do but `open` the bundle. A locally built bundle is never quarantined, so no signing and no Gatekeeper prompt. The launched app *is* the registered handler, so no forwarder: Launch Services reuses the running bundle and fires `open-url` — which can land before the renderer exists, hence `#deliver` parks it until `takePending` marks the renderer live.
...
- `getSwitchStatus(name)` / `activateSwitch(name)` / `deactivateSwitch(name)` / `setSwitchKeepActive(name, value)` — one generic surface for the StatusBar switches, backed by `Switchers` (`src/main/services/switchers/`). Each `*Switch` holds pure feature logic — `status()` plus `activate()`/`deactivate()` that do the work or **throw** a friendly message; `Switchers.#attempt` maps every outcome to fresh status plus an optional `error` string alerted by the tooltip button. A new switch is a new `*Switch` class plus a `#switches` map entry. The path-installing switches (proxy, statusline) carry a "Keep active after close" checkbox next to their tooltip button (`setSwitchKeepActive(name, value)` — in-memory per run, every launch starts checked); switches not kept active are deactivated on quit (`Switchers.deactivateOnQuit`, wired to `will-quit` in main.js). An `npx claude-discover` run executes from the throwaway `_npx` cache (`IS_EPHEMERAL` in `paths.js`): its checkbox is disabled and `activate()` clears keep-active, so settings.json commands never outlive the cache dir — while a switch the run never activated (another instance's, or a previous run's) stays checked and survives its quit. `StatusSwitch.jsx`'s `useSwitch` polls status every 5s so external settings.json edits show up live; a `StatusSwitch` given a custom `button` label renders an action button that always activates.
  - **proxy** — `{ running, configured }` (ping probe + whether `env.ANTHROPIC_BASE_URL` points at the proxy); configured + not running ⇒ Claude Code can't reach the API, shown as ⚠ DOWN. Activate spawns the proxy detached via `ELECTRON_RUN_AS_NODE` (so it outlives the app) — forked as a utility process, from which proxy.mjs re-spawns itself detached and exits, never straight from the main process: a direct child inherits Chromium's inheritable sockets on Windows (the dev CDP server on 9333) and keeps the port bound after the app quits, while utility processes get a clean handle table — and writes settings.json (env keys + revive hook) only once the proxy is up — a failed activate never changes it, and a foreign base URL is never overwritten (activate errors instead). Deactivate exits the proxy through the control route and removes the env keys and hook. `ProxySwitch` is the sole owner of that config; the proxy's port, route paths and ping body are defined once in `bin/proxy.config.js` (the only proxy file the app imports; the renderer doesn't import from bin/, so StatusBar's tooltip hardcodes the URL).
...
- `onDeepLink(cb)` / `takeDeepLink()` — a `claude-discover://` link resolved to `{ id, date }`: pushed while running, pulled once on mount for the link that cold-started the app (see Deep linking).
